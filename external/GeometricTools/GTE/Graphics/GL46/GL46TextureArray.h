...
namespace gte
{
    class GL46TextureArray : public GL46Texture
    {
    public:
        // Abstract base class, a shim to distinguish between single textures
        // and texture arrays.
        virtual ~GL46TextureArray();
    protected:
        // No public construction.  Derived classes use this constructor.
        GL46TextureArray(TextureArray const* gtTexture, GLenum target, GLenum targetBinding);
    ...
    public:
        ...
        void CopyLevelGpuToGpu(GL46TextureArray* target, uint32_t item, uint32_t level)
        {
            (void)target;
            (void)item;
            (void)level;
