    ...
    DX11TextureArray(textureCubeArray)
{
    ...
    uint32_t usage = textureCubeArray->GetUsage();
    ...
    {
        uint32_t const numSubresources = textureCubeArray->GetNumSubresources();
    ...
    if (textureCubeArray->GetUsage() == Resource::Usage::SHADER_OUTPUT)
    {
        CreateUAView(device, desc);
    }

    // Create a staging texture if requested.
    if (textureCubeArray->GetCopy() != Resource::Copy::NONE)
    {
...
void DX11TextureCubeArray::CreateStaging(ID3D11Device* device, D3D11_TEXTURE2D_DESC const& tx)
{
    ...
    desc.CPUAccessFlags = msStagingAccess[GetTextureArray()->GetCopy()];
