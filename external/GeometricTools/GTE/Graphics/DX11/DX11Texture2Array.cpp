    ...
    DX11TextureArray(textureArray)
{
    ...
    uint32_t usage = textureArray->GetUsage();
    ...
    {
        uint32_t const numSubresources = textureArray->GetNumSubresources();
    ...
    if (textureArray->GetUsage() == Resource::Usage::SHADER_OUTPUT)
    {
        CreateUAView(device, desc);
    }

    // Create a staging texture if requested.
    if (textureArray->GetCopy() != Resource::Copy::NONE)
    {
