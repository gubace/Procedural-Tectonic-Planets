    ...
    DX11TextureArray(textureCube)
{
    ...
    uint32_t usage = textureCube->GetUsage();
    ...
    {
        uint32_t const numSubresources =
    ...
    if (textureCube->GetUsage() == Resource::Usage::SHADER_OUTPUT)
    {
        CreateUAView(device, desc);
    }

    // Create a staging texture if requested.
    if (textureCube->GetCopy() != Resource::Copy::NONE)
    {
