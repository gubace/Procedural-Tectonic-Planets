...
namespace gte
{
    // Type T is the floating-point type of the components; it must be
    // 'float' or 'double'.
    template <typename T, typename IndexType>
    class MinimumVolumeBox3<T, IndexType, MVB3FloatingPoint>
    {
    public:
    ...
    protected:
        ...
        void CreateMeshTopology(std::size_t numTriangles, IndexType const* indices, VETManifoldMesh& mesh)
        {
            ...
            mTVertices.resize(vMap.size());
            mTNormals.resize(tMap.size());
        ...
        void ExtractMeshGeometry(std::size_t numVertices, TVector3 const* vertices)
        {
            ...
            {
                mTVertices[i] = vertices[i] - mTOrigin;
                for (std::int32_t j = 0; j < 3; ++j)
                {
            ...
            // errors. This is necessary for creating the vertex adjacency
            // data structure that supports the hill-climbing algorithm that
            // computes extreme hull points in a specified direction. The
            // mTNormals[] are normalized so that the floating-point-valued
            // dot products in the hill climbing are computed to reduce 
            ...
            for (std::size_t i = 0; i < mTriangles.size(); ++i)
            {
                ...
                for (std::int32_t j = 0; j < 3; ++j)
                {
                    mTNormals[i][j] = static_cast<T>(mNNormals[i][j]);
                }
                Normalize(mTNormals[i]);
            }
        }
        ...
        void ComputeAlignedCandidate()
        {
            ...
            TVector3 diff = pmax - pmin;
            mAlignedCandidate.volume = diff[0] * diff[1] * diff[2];
        }

        std::size_t GetExtreme(TVector3 const& direction, T& dMax)
        {
            std::size_t vMax = mVClimbStart;
            dMax = Dot(direction, mTVertices[vMax]);

            for (std::size_t i = 0; i < mTVertices.size(); ++i)
            {
                ...
                for (std::size_t j = 1; j <= numAdjacent; ++j)
                {
                    ...
                    T dCandidate = Dot(direction, mTVertices[vCandidate]);
        ...
        void ComputeVolume(Candidate& candidate)
        {
            ...
            TVector3 diff = pmax - pmin;
            candidate.volume = diff[0] * diff[1] * diff[2] /
                Dot(candidate.axis[2], candidate.axis[2]);
        }
        ...
        void GetMinimumVolumeBox(OrientedBox3<T>& box, T& volume)
        {
            ...
            RVector3 rCenter{}, rPMin{}, rPMax{}, rVertexMin{}, rVertexMax{};;
            ...
            for (std::int32_t i = 0; i < 3; ++i)
            {
                ...
                for (std::int32_t j = 0; j < 3; ++j)
                {
                    rVertexMin[j] = mNVertices[mvc.minSupportIndex[i]][j];
                    rVertexMax[j] = mNVertices[mvc.maxSupportIndex[i]][j];
                    rAxis[i][j] = mvc.axis[i][j];
                }
                rSqrLengthAxis[i] = Dot(rAxis[i], rAxis[i]);

                rPMin[i] = Dot(rAxis[i], rVertexMin);
                rPMax[i] = Dot(rAxis[i], rVertexMax);
            }
        ...
        std::vector<TVector3> mTVertices;
        std::vector<TVector3> mTNormals;
        TVector3 mTOrigin;
    ...
    protected:
        ...
        {
            T const half = static_cast<Number>(0.5);
            std::vector<T> t(mMaxSample + 1);
            t[0] = static_cast<T>(0);
            t[mMaxSample] = static_cast<T>(1);
        ...
        {
            T const half = static_cast<T>(0.5);
            std::vector<T> s(mMaxSample + 1);
            s[0] = static_cast<T>(0);
            s[mMaxSample] = static_cast<T>(1);
            for (auto const& item : mDomainIndex)
            {
        ...
        virtual void MinimizerVariableS(
            T const& sminNumer, T const& smaxNumer, T const& sDenom,
            Candidate& c, Candidate& mvc)
        {
            T const half = static_cast<T>(0.5);
            std::vector<T> s(mMaxSample + 1), oms(mMaxSample + 1);
            s[0] = sminNumer;
            ...
            for (std::size_t i = 0; i <= mMaxSample; ++i)
            {
                ...
                T q0 = oms[i] * c.f00 + s[i] * c.f10;
                T q1 = oms[i] * c.f01 + s[i] * c.f11;
                if (q0 > q1)
                {
        ...
        {
            T const half = static_cast<T>(0.5);
            std::vector<T> t(mMaxSample + 1), omt(mMaxSample + 1);
            t[0] = tminNumer;
            ...
            for (std::size_t i = 0; i <= mMaxSample; ++i)
            {
                T p0 = omt[i] * c.f00 + t[i] * c.f01;
                T p1 = omt[i] * c.f10 + t[i] * c.f11;
                if (p0 > p1)
                {
